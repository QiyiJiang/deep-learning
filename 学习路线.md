...
- **操作**：克隆 MiniMind 仓库（`git clone https://github.com/jingyaogong/minimind.git`），创建实验分支；安装依赖（Python、PyTorch、CUDA、常用库，如 torchviz, netron, triton, tensorboard, nvitop, swanlab 等）。
- **分析**：检查 GPU/CPU 可用情况，CUDA 驱动与显存配置；了解 README 中的最小训练/推理命令（注意 `trainer/` 目录下的脚本）。
- **产出**：环境依赖安装记录。

#### Day 2: 基线训练复现

- **操作**：运行项目默认训练脚本（例如 `python trainer/train_pretrain.py`），观察是否正常启动；使用 SwanLab 记录训练过程日志（loss 曲线、吞吐、显存峰值）。
...
- **操作**：编写 `debug_graph.py` 脚本，实例化模型 `MiniMindForCausalLM`，构造一个小批量 Dummy 输入（如 `input_ids = torch.randint(0, 100, (1, 10))`），执行一次前向并使用 `torchviz.make_dot` 生成计算图 PDF。
- **分析**：在生成的计算图中定位 Attention、MLP 等模块，观察注意力中的 Q、K、V 是如何从一个线性层（`qkv_proj`）中分离出来的；如果是 MoE 模型，观察专家路由（Gating）的连接方式。
- **产出**：`model_graph.pdf`（计算图文件），并在图中标注出 Attention 和 MLP (或 MoE) 的模块边界。

#### Day 2: 手推张量形状（Shape Inference）

- **操作**：打开 `model/model_minimind.py`，新建一个表格（Excel 或 MarkDown 表格），列出每一层操作的名称、输入形状、输出形状、是否发生显存拷贝等。逐行分析 forward 函数中各步骤的形状变化。
- **分析**：重点关注 `view`、`reshape`、`transpose` 等操作是否会触发内存复制；在 Attention 计算中，理解 `qkv_proj` 后张量的分头（multi-head）布局；观察 MoE 中 `top_k` 专家选择后的张量分发逻辑。
...
#### Day 4: RoPE 旋转编码与 YaRN 外推

- **操作**：定位 `model/model_minimind.py` 中 RoPE 实现（`precompute_freqs_cis`），打印不同序列长度下的 `freqs_cos`；观察开启 `inference_rope_scaling` (YaRN) 后的频率变化。
- **分析**：理解 RoPE 是如何将向量两两配对；分析 YaRN 算法如何通过插值（Interpolation）实现免微调的长度外推（从 2048 到 32768）。
- **产出**：一段注释或报告，解释 RoPE 的内存布局及 YaRN 的缩放原理。
...
#### Day 1: 数据流程与 ChatML 模板

- **操作**：阅读 `dataset/` 目录下的数据处理脚本（如 `lm_dataset.py`），绘制"Jsonl 语料→ChatML 模板应用→Tokenize→Dynamic Mask"流程图。
- **分析**：了解 ChatML 模板（`<|im_start|>` 等）如何构造对话；重点分析 `SFTDataset` 中的 `_generate_loss_mask` 逻辑，理解为什么只对 `assistant` 的输出计算 Loss。
- **产出**：数据流程图文档（如 Markdown + 图），标注模板拼接后的 token 序列结构。

#### Day 2: 动态 Mask 与 Padding 实验

- **操作**：修改 `SFTDataset`，观察不同 `max_length` 对 padding 数量的影响；在 `__getitem__` 中打印一个样本的 token 与 mask 对应关系。
- **分析**：理解 `loss_mask` 如何过滤掉 `user` 指令和 `padding` 部分的梯度，确保模型只学习生成回答。
- **产出**：样本 token-mask 对应表，记录不同设置下的训练吞吐。

#### Day 3: DataLoader 瓶颈定位与 SwanLab 监控

- **操作**：在训练脚本中开启 `swanlab` 监控，使用 `py-spy` 对训练过程进行采样分析。
- **分析**：检查是否存在 CPU Tokenize 阶段过重；观察 SwanLab 中的吞吐曲线，找出主要的时间消耗。
- **产出**：CPU 火焰图截图及 SwanLab 训练监控报告。
...
#### Day 1: 训练过程监控 (SwanLab)

- **操作**：在 `trainer/` 脚本中配置 `swanlab`，记录训练过程中的 **吞吐率**、**Loss 曲线** 和 **显存占用**。
- **分析**：运行一次训练，观察 SwanLab 实时图表，找出是前向、反向还是更新阶段最耗时。
- **产出**：基础训练监控报告（SwanLab 链接或截图）。

#### Day 2: 断点续训 (Resume) 验证

- **操作**：在训练过程中人为中断程序，使用 `--from_resume 1` 参数重新启动训练。
- **分析**：观察控制台日志，确认模型是否正确加载了 `checkpoints/` 下的 `.pth` 文件、优化器状态及训练进度（Step/Epoch）；检查 SwanLab 曲线是否保持连续。
- **产出**：断点续训成功日志与曲线连续性证明。

#### Day 3: FP32 vs AMP (混合精度) 对比

- **操作**：分别在 FP32 模式和 AMP 模式下运行训练，记录速度、显存占用和最终模型稳定性。
- **分析**：比较两者的训练速度提升率、显存节省幅度以及收敛差异。
- **产出**：对比实验结果（表格或曲线）。
